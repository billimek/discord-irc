...
class Bot {
  ...
  sendToIRC(message) {
    ...
    const channelName = `${message.channel.server.id},#${message.channel.name}`;
  ...
  sendToDiscord(author, channel, text) {
    ...
    if (discordServerChannelName) {
      ...
      const discordServerID = discordServerChannelName.split(',')[0];
      const discordChannelName = discordServerChannelName.split(',')[1].toLowerCase();
      if (this.discord.servers.get('id', discordServerID)) {
        const server = this.discord.servers.get('id', discordServerID);
        const discordChannel = server.channels.get('name', discordChannelName.slice(1));
